...
Unified main script for DrivAerNet continual learning experiments.
"""
...
from DrivAerNet_Benchmark import (
    create_drivaernet_benchmark,
    get_drivaernet_benchmark,
    get_drivaernet_plus_bin_benchmark, 
    get_drivaernet_plus_class_benchmark
)
...
def get_benchmark(args):
    """Get the appropriate benchmark based on dataset and scenario."""
    if args.dataset == 'drivaernet':
        return get_drivaernet_benchmark()
    elif args.dataset == 'drivaernet_plus' and args.scenario == 'bin':
        return get_drivaernet_plus_bin_benchmark()
    elif args.dataset == 'drivaernet_plus' and args.scenario == 'input':
        return get_drivaernet_plus_class_benchmark()
    else:
        raise ValueError(f"Invalid dataset/scenario combination: {args.dataset}/{args.scenario}")
...
def create_cl_strategy(args, model, optimizer, criterion, eval_plugin, device):
    ...
    # Get the LR configuration for DrivAerNet benchmark
    lr_config = BenchmarkLRConfigs.shapenet_cars()  # Reuse ShapeNet config for now
...
def run_single_iteration(args, io, iteration):
    ...
    try:
        ...
        # Determine number of classes based on dataset/scenario
        if args.dataset == 'drivaernet' or (args.dataset == 'drivaernet_plus' and args.scenario == 'bin'):
            num_classes = 4
        else:  # drivaernet_plus with class scenario
            num_classes = 6
        ...
        print(f"Dataset: {args.dataset}, Scenario: {args.scenario}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DrivAerNet Continual Learning')
    
    # Dataset settings
    parser.add_argument('--dataset', type=str, default='drivaernet', 
                       choices=['drivaernet', 'drivaernet_plus'],
                       help='Dataset variant to use')
    parser.add_argument('--scenario', type=str, default='bin',
                       choices=['bin', 'input'],
                       help='Incremental scenario (class only valid for drivaernet_plus)')
    ...
    # Validate dataset/scenario combination
    if args.dataset == 'drivaernet' and args.scenario != 'bin':
        raise ValueError("DrivAerNet only supports 'bin' scenario")
...
# DrivAerNet standard (bin incremental, 4 classes):
# python main.py --dataset drivaernet --scenario bin --exp_name 'DrivAerNet_naive_test' --strategy 'naive' --epochs 2 --num_iters 1 --batch_size 8

# DrivAerNet++ bin incremental (4 classes):
# python main.py --dataset drivaernet_plus --scenario bin --exp_name 'DrivAerNet_Plus_Bin_replay' --strategy 'replay' --mem_size 1000 --epochs 200 --num_iters 4 --batch_size 8

# DrivAerNet++ class incremental (6 classes):
# python main.py --dataset drivaernet_plus --scenario class --exp_name 'DrivAerNet_Plus_Class_ewc' --strategy 'ewc' --ewc_lambda 0.01 --epochs 150 --batch_size 12

# Pretrained model example:
# python main.py --dataset drivaernet --scenario bin --exp_name 'DrivAerNet_PT_baseline' --strategy 'naive' --pretrained=True --model_path='models/PN_best.pth' --epochs=100 --num_iters 1 --batch_size 8
# Example usage
    # python main.py --exp_name 'DRIVAERNET_replay_pn20k_e200' --strategy 'replay' --mem_size=1000 --epochs=200 --num_iters 4 --batch_size 8 
    # python main.py --exp_name 'DRIVAERNET_replay_pn20k_e200' --strategy 'gem' --mem_size=1000 --epochs=200 --num_iters 4 --batch_size 8
    # python main.py --exp_name 'DRIVAERNET_debug' --strategy 'naive' --mem_size=1000 --epochs=200 --num_iters 4 --batch_size 12
    
    # Pretrained model
    # python main.py --exp_name 'DRIVAERNET_PT_baseline_pn20k_e200' --strategy 'naive' --pretrained=True --model_path='models/PN_best.pth' --epochs=200 --num_iters 1 --batch_size 8 
    # python main.py --exp_name 'DRIVAERNET_exptest' --strategy 'naive' --pretrained=True --model_path='models/PN_best.pth' --epochs=1 --num_iters 1 --batch_size 8 
